...
* **Transmettez des valeurs d’attribut, et non des libellés d’affichage.** Les valeurs d’attribut de contexte sont **sensibles à la casse**. Transmettez la valeur brute envoyée par votre application ou site web (par exemple, `"en_US"` ou `"ANDROID"`), et non le libellé affiché dans la console.
