...
description: Découvrez les attributs spécifiques au visiteur qui sont stockés dans le profil du visiteur afin de fournir des informations à ce sujet qui peuvent être utilisées dans vos activités d’Adobe [!DNL Target] .
...
Les attributs de profil dans [!DNL Adobe Target] sont des paramètres spécifiques à un visiteur. Ces attributs sont stockés dans le profil du visiteur pour fournir des informations sur le visiteur, qui peuvent être utilisées dans vos activités.

Un profil utilisateur contient des informations démographiques et comportementales sur le visiteur d’une page web. Ces informations peuvent inclure l’âge, le sexe, les produits achetés, la dernière heure de la visite, etc. que [!DNL Target] utilise pour personnaliser le contenu qu’il diffuse au visiteur.
...
Pour copier un script de profil existant, dans la liste [!UICONTROL Scripts de profil] , cliquez sur l’icône représentant des points de suspension pour le script souhaité, puis sur **[!UICONTROL Dupliquer]**.
...
Par exemple, la carte d’informations de script de profil suivante est accessible en cliquant sur l’icône [!UICONTROL Infos] du script de profil souhaité dans la liste ([!UICONTROL Audiences] > [!UICONTROL Scripts de profil]).

L’onglet [!UICONTROL Informations sur le script] contient les informations suivantes : Nom, Description et code de script.

![Carte d’informations du script de profil](assets/profile_script_info_card.png)

Cliquez sur **[!UICONTROL Afficher tous les détails]** pour afficher les audiences et les activités qui référencent le script de profil sélectionné.

![Carte d’informations du script de profil > Onglet Utilisation du script](assets/profile_script_info_card_usage_tab.png)

>[!NOTE]
>
>L’onglet [!UICONTROL Utilisation du script] n’affiche pas les activités qui référencent le script de profil sélectionné dans les situations suivantes :
>
> * L’activité se trouve à l’état [!UICONTROL Version préliminaire] .
...
>[!NOTE]
>
>L’utilisation de [!DNL console.log] dans un script de profil ne génère pas la valeur du profil, car les scripts de profil s’exécutent côté serveur.
...
* **Ajoutez des scripts de profil comme jetons de réponse pour déboguer les scripts de profil :**
   ...
   Chaque fois que vous chargez une page pour votre site avec [!DNL Target] dessus, une partie de la réponse de [!DNL Target] contient votre valeur pour le script de profil donné, comme illustré ci-dessous :
...
Les scripts de profil ne parviennent pas à lire directement la page, car ils sont exécutés côté serveur. Les données doivent passer par une requête de mbox ou d’autres [méthodes de transfert de données dans Target](/help/c-implementing-target/c-considerations-before-you-implement-target/c-methods-to-get-data-into-target/methods-to-get-data-into-target.md#concept_0069C0EFB56C4700BB33F2F35C2B9B17). Une fois que les données se trouvent dans [!DNL Target], les scripts de profil peuvent lire les données sous la forme d’un paramètre de mbox ou de profil.

## Référence JavaScript pour les paramètres de profil de script

Des connaissances JavaScript simples sont requises pour utiliser efficacement le profil de script
...
Crée une variable pour la mesure du jour en millisecondes. Si le nom de la mbox est `orderThankyouPage`, définissez un attribut de profil utilisateur local (invisible) nommé `lastPurchaseTime` pour afficher la valeur de la date et de l’heure actuelles. La valeur de l’heure du dernier achat est lue et, si elle est définie, la valeur [!DNL Target] renvoie le temps qui s’est écoulé depuis l’heure du dernier achat, divisé par le nombre de millisecondes par jour (ce qui donne le nombre de jours depuis le dernier achat).
...
Crée une variable appelée `frequency`, en l’initialisant à la valeur précédente ou à 0, s’il n’y avait aucune valeur précédente. Si le nom de la mbox est `orderThankyouPage`, la valeur incrémentée est renvoyée.
...
Crée une variable appelée `adobeQA` pour effectuer le suivi d’un utilisateur pour [l’AQ d’activité](/help/c-activities/c-activity-qa/activity-qa.md).
...
| `<=` | Identique à `<` sauf si les variables sont égales, elles sont évaluées sur vraies. |
| `>=` | Identique à `>` sauf si les variables sont égales, elles sont évaluées sur vraies. |
