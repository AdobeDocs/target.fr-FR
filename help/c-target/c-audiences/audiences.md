...
seo-description: Les audiences déterminent qui verra le contenu et les expériences d’une activité ciblée.
seo-title: Créer des audiences
solution: Target
...
Les audiences sont utilisées partout où le ciblage est disponible. Lorsque vous ciblez une activité, vous pouvez sélectionner une audience réutilisable de la liste [!UICONTROL d’audiences], créer une audience spécifique à l’activité et la cibler, ou  [combiner plusieurs audiences](../../c-target/combining-multiple-audiences.md#concept_A7386F1EA4394BD2AB72399C225981E5) pour créer une audience ad hoc.
...
* **Audiences avec création de rapports :** Utilisé pour déterminer comment différents types de visiteurs répondent au même contenu pour vous permettre d’analyser vos résultats de test.
   ...
   Dans [!DNL Target], vous pouvez configurer les audiences avec création de rapports seulement si vous utilisez [!DNL Target] comme source des rapports. Si vous utilisez [[!DNL Adobe Analytics] comme source des rapports (A4T)](../../c-integrating-target-with-mac/a4t/a4t.md#concept_7540C8C04259434AB6EE33B09F47A1DE), vous devez configurer les audiences avec création de rapports dans [!DNL Analytics].
...
Vous pouvez également cibler des paramètres de profil personnalisés et des paramètres `user.`. Lors de l’ajout d’une audience, cliquez sur **[!UICONTROL Profil du visiteur]**, puis sous [!UICONTROL Paramètres utilisateur personnalisés] ou [!UICONTROL Paramètres de profil personnalisés] dans la liste déroulante [!UICONTROL Profil du visiteur], sélectionnez le paramètre que vous souhaitez utiliser pour cibler votre activité. Si le paramètre souhaité ne s’affiche pas, il n’a pas été déclenché par une mbox. D’autres paramètres mbox personnalisés sont disponibles dans la liste déroulante [!UICONTROL Paramètres personnalisés].
...
La carte de définition de l’audience suivante est destinée à une audience importée depuis Adobe Experience Cloud. Dans ce cas, l’audience a été importée depuis Adobe Audience Manager (AAM). Notez que Target ne fournit aucune information détaillée sur la définition de l’audience car ces informations ne sont pas présentes dans Target.
