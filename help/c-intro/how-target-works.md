...
seo-description: 'Il est possible d’intégrer Adobe Target à des sites web par le biais d’une de ces deux bibliothèques JavaScript : at.js ou mbox.js'
seo-title: Fonctionnement d’Adobe Target
solution: Target
...
Vous devez référencer un des fichiers JavaScript de Target sur chaque page de votre site. Vous pouvez par exemple l’ajouter à votre en-tête global.
...
4. Le contenu s’affiche en fonction des règles de votre campagne.
...
En vue d’améliorer le temps de réponse, les environnements Edge hébergent uniquement la logique d’activité et les informations sur le profil et les offres en cache. Les bases de données des activités et du contenu, les données [!DNL Analytics], les API et les interfaces utilisateurs des spécialistes du marketing sont hébergées dans les environnements de données centraux d’Adobe. Des mises à jour sont alors envoyées aux nœuds Edge. Les environnements centraux et les nœuds Edge sont automatiquement synchronisés en vue de continuellement mettre à jour les données d’activité mises en cache. La modélisation 1:1 est également stockée à chaque nœud, afin que les demandes les plus complexes puissent aussi demeurer sur le réseau Edge.
...
>[!IMPORTANT]
>
>[!DNL Adobe Target] ne dispose pas actuellement d&#39;un réseau Edge en Chine et que les performances de l&#39;utilisateur final continueront à être limitées pour [!DNL Target] les clients en Chine. Because of the Great Firewall and the lack of Edge nodes within the country, the experiences of sites with [!DNL Target] deployed will be slow to render and page loads will be affected. Also, the [!DNL Target] user interface might also experience latency.
...
Aucune modification n’est apportée à la page si le navigateur de l’utilisateur ne peut pas se connecter au cours du délai d’attente défini (par défaut, 15 secondes). Si ce délai est écoulé, un paramètre du cookie est modifié afin que l’utilisateur voie immédiatement le contenu par défaut pour tous les autres emplacements. Cet état perdure pendant une demi-heure, après quoi le navigateur de l’utilisateur tente à nouveau de contacter les serveurs d’Adobe pour les requêtes de contenu.
...
Google indique que suivre ces directives « devrait avoir pour vos tests un léger impact, voire aucun, sur votre site dans les résultats de recherche ».
