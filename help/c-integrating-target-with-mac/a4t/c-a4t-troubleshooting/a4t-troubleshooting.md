...
title: Comment résoudre les problèmes liés à l’intégration Analytics et  [!DNL Target] A4T ?
...
Cette rubrique aborde certains problèmes courants qui se produisent lors de l’utilisation de [!DNL Adobe Analytics] comme source des rapports pour [!DNL Adobe Target] (A4T).
...
Si vous utilisez plusieurs sources de création de rapports, les suites de rapports doivent également être présentes dans la source de création de rapports par défaut définie dans [!DNL Target]. Si les suites de rapports ne figurent pas dans la source de création de rapports par défaut, elles ne s’affichent pas.
...
* **at.js 2.x** : Dans l’ [!DNL Target] appel à , l’ID supplémentaire est renvoyé dans l’en-tête HTTP comme valeur de  `experienceCloud.analytics.supplementalDataId`. Dans l’appel à [!DNL Analytics], l’ID supplémentaire est contenu dans le paramètre `sdid`.
