...
description: Trouvez des réponses aux questions sur l’utilisation des offres de redirection lors de l’utilisation d’Analytics pour les activités  [!DNL Target] (A4T). A4T lets you use Analytics reporting for [!DNL Target] .
...
Oui, si votre mise en oeuvre utilise [!DNL at.js]. Toutefois, votre implémentation doit respecter la configuration minimale requise ci-dessous pour utiliser les offres [de redirection](/help/c-experiences/c-manage-content/offer-redirect.md#task_33C80CD722564303B687948261484F94) dans les activités qui utilisent Analytics comme source de création de rapports.
...
Avec [!DNL at.js] version 0.9.6 (ou ultérieure) et [!DNL AppMeasurement.js] 2.1 (ou version ultérieure), la requête [!DNL Analytics] de la nouvelle page signale une URL de référence de [!DNL `www.google.com`].
...
## ![Badge du SDK Web Adobe Experience Platform ](/help/assets/platform.png) La  [!DNL Adobe Experience Platform Web SDK] prise en charge des offres de redirection pour A4T est-elle prise en charge ? {#platform}

Les questions fréquentes suivantes fournissent des informations supplémentaires sur l’utilisation d’A4T et les offres de redirection avec la balise [!DNL Platform Web SDK].

### Analytics for Target (A4T) prend-il en charge les offres de redirection ?

Oui, A4T via le SDK Web Platform prend en charge [les offres de redirection](/help/c-experiences/c-manage-content/offer-redirect.md).

### Le [!UICONTROL compositeur d’expérience visuelle] (VEC) et le [!UICONTROL compositeur d’expérience d’après les formulaires] sont-ils pris en charge ?
...
### Puis-je utiliser des offres de redirection HTML/personnalisées avec la balise [!DNL Platform Web SDK] ?

Non, vous devez utiliser une offre de redirection intégrée pour les activités qui utilisent A4T. Du point de vue [!DNL Target], les offres HTML sont opaques. [!DNL Target] ne peut pas savoir qu’un élément HTML particulier contient du code JavaScript qui instancie une redirection.
