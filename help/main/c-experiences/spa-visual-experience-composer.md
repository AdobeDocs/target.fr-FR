...
En [!DNL Adobe Target], le [!UICONTROL compositeur d’expérience visuelle] (VEC) permet aux marketeurs de créer eux-mêmes des activités et de personnaliser les expériences qui peuvent être diffusées dynamiquement dans les applications multi-pages traditionnelles via la mbox globale d’Adobe Target. Toutefois, cela dépend de la récupération des offres au chargement de la page ou des appels serveur suivants, qui introduit une latence, comme illustré dans le diagramme ci-dessous. Cette approche ne fonctionne pas bien avec les applications monopages, car elle altère l’expérience utilisateur et les performances de l’application.
...
Pour exécuter ces objectifs, dans le VEC, cliquez sur [!UICONTROL Composer] et appliquez ces modifications à la vue d’accueil.
...
1. Cliquez sur [!UICONTROL Parcourir], puis sur le lien [!UICONTROL Produits] au niveau de l’en-tête.
1. Cliquez sur [!UICONTROL Charger plus] une fois pour accéder à la deuxième ligne de produits.
...
Les paramètres [!UICONTROL Diffusion de page] vous permettent de configurer des règles afin de déterminer à quel moment une activité Target doit être qualifiée et exécutée pour une audience.

Pour accéder aux options [!UICONTROL Diffusion de page] dans le workflow en trois parties de création guidée d’activités du compositeur d’expérience visuelle, à l’étape **[!UICONTROL Expériences]**, cliquez sur **[!UICONTROL Configurer]** (icône d’engrenage) > **[!UICONTROL Diffusion de page]**.
...
Ce message s’affiche lorsque vous ajoutez la première action à une vue pour chaque nouvelle activité que vous créez. Ce message permet de s’assurer que tous les membres de votre organisation apprennent à appliquer correctement ces règles [!UICONTROL Diffusion de page].
