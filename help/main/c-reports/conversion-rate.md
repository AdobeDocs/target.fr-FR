---
keywords: Ciblage
description: Découvrez comment Adobe  [!DNL Target]  et calcule le taux de conversion, l’effet élévateur, le degré de confiance et l’intervalle de confiance pour chaque expérience.
title: Comment puis-je afficher le taux de conversion, l’effet élévateur et le niveau de confiance ?
feature: Reports
...
L’illustration suivante présente l’en-tête de graphique d’un exemple d’activité avec les en-têtes [!UICONTROL Conversion Rate], [!UICONTROL Lift] et [!UICONTROL Confidence] en surbrillance.

![image du taux de conversion](assets/conversion-rate.jpg)
...
![image taux-de-conversion-détail](assets/conversion-rate-detail.jpg)
...
Les données AOV, RPV et Ventes s’affichent pour chaque expérience si vous avez inséré une mbox Placer une commande (`orderConfirmPage`) et l’avez sélectionnée en tant que mbox de conversion.
...
### Confiance {#section_26FE5E44BDD5478792A65FCFD83DCCDC}

Le degré de confiance d’une expérience ou d’une offre affichée est une probabilité (exprimée en pourcentage) d’obtenir un résultat moins extrême que celui qui est réellement observé, si l’hypothèse nulle est vraie, (en substance, s’il n’y a aucune différence de taux de conversion entre cette expérience ou offre, et l’expérience/offre de contrôle). En termes de p-values, ce degré de confiance affiché est 1 - p-value. En d’autres termes, un degré de confiance plus élevé indique que les données sont moins cohérentes avec l’hypothèse que l’offre/expérience de contrôle et de non-contrôle ont des taux de conversion égaux.
...
L’*intervalle de confiance* est une plage d’estimations dans laquelle la valeur réelle de la mesure peut être trouvée à un niveau de confiance donné. Target affiche toujours des intervalles de confiance à 95 %. L’intervalle de confiance est représenté par un pourcentage +/- gris clair dans la colonne Taux de conversion. Dans l’exemple ci-dessous, l’intervalle de confiance pour l’effet élévateur de l’expérience B est de plus ou moins 15,65 %.

![image conversion_rate](assets/conversion_rate.png)

**Exemple :** la valeur RPV observée d’une expérience est de 10 $ et son **intervalle de confiance** à 95 % est compris entre 5 et 15 $. Inconnu de nous, son vrai RPV est de 12 $. Ensuite, si nous avons exécuté ce test plusieurs fois, 95 % du temps pendant lequel l’intervalle de confiance que nous calculons contiendra la valeur _true_ de la valeur RPV de 12 $.
...
Pour calculer ces mesures calculées, téléchargez le fichier Excel [Calculateur de confiance complet](/help/main/assets/complete_confidence_calculator.xlsx) de Target pour saisir la valeur de l’activité, ou consultez [Calculs statistiques dans des tests A/B](/help/main/c-reports/statistical-methodology/statistical-calculations.md).
...
## Pourquoi recommande-[!DNL Target] d’utiliser les tests en t de Welch ? {#t-test}

Les tests A/B sont des expériences visant à comparer la valeur moyenne d’une mesure commerciale dans une variante de contrôle (également appelée expérience) à la valeur moyenne de cette même mesure dans une ou plusieurs expériences alternatives.

[!DNL Target] recommande d’utiliser le test t de [Welch](https://en.wikipedia.org/wiki/Welch%27s_t-test), car il nécessite moins d’hypothèses que des alternatives telles que les tests z, et constitue le test statistique approprié pour effectuer des comparaisons par paires de mesures commerciales (quantitatives) entre des expériences de contrôle et des expériences alternatives.

### Plus en détail

Lors de l’exécution de tests A/B en ligne, chaque utilisateur/visiteur est affecté de manière aléatoire à une seule variante. Par la suite, nous effectuons des mesures des mesures commerciales d’intérêt (par exemple, conversions, commandes, chiffre d’affaires, etc.) pour les visiteurs dans chaque variante. Le test statistique que nous utilisons teste ensuite l’hypothèse que la mesure commerciale moyenne (par exemple, le taux de conversion, les commandes par utilisateur, les recettes par utilisateur, etc.) est égale pour le contrôle et une variante alternative donnée.

Bien que la mesure commerciale elle-même puisse être distribuée selon une distribution arbitraire, la distribution de la moyenne de cette mesure (dans chaque variante) doit converger vers une distribution normale via le [théorème de limite centrale](https://en.wikipedia.org/wiki/Central_limit_theorem). Notez que bien qu’il n’y ait aucune garantie sur la vitesse à laquelle cette distribution d’échantillonnage de la moyenne convergera vers la normale, cette condition est généralement atteinte compte tenu de l’échelle des visiteurs dans les tests en ligne.

Compte tenu de cette normalité de la moyenne, on peut montrer que la statistique de test à utiliser suit une distribution t, car il s’agit du rapport d’une valeur normalement distribuée (la différence dans la moyenne de la mesure commerciale) à un terme de mise à l’échelle basé sur une estimation à partir des données (l’erreur standard de la différence dans les moyennes). Le **test t** est alors le test d&#39;hypothèse approprié, étant donné que la statistique du test suit une distribution t.

### Pourquoi d’autres tests ne sont pas utilisés

Un test **z** est techniquement inapproprié, car dans le scénario typique du test A/B, le dénominateur de la statistique du test n’est pas dérivé d’une variance connue et doit plutôt être estimé à partir des données. Toutefois, pour des échantillons de taille suffisante, les tests z et t sont identiques.

Les tests **chi carré** ne sont pas utilisés car ils sont appropriés pour déterminer s’il existe une relation qualitative entre deux variantes (c’est-à-dire une hypothèse nulle qu’il n’y a aucune différence entre les variantes). Les tests en T sont plus appropriés pour le scénario de comparaison _quantitative_ des mesures.

Le **test U de Mann-Whitney** est un test non paramétrique, approprié lorsque la distribution d’échantillonnage de la mesure commerciale moyenne (pour chaque variante) n’est normalement pas distribuée. Cependant, comme nous l’avons vu précédemment, étant donné l’ampleur du trafic impliqué dans les tests en ligne, le théorème de la limite centrale s’applique généralement, et le test en t peut donc être appliqué en toute sécurité.

Des méthodes plus complexes telles que **ANOVA** (qui généralisent des tests en t à plus de deux variantes) peuvent être appliquées lorsqu’un test comporte plus de deux expériences (« tests A/B »). Cependant, ANOVA répond à la question de « si toutes les variantes ont la même moyenne », alors que dans le test A/Bn typique, nous sommes plus intéressés par _quelle variante spécifique_ est la meilleure. En [!DNL Target], nous appliquons donc des tests t réguliers comparant chaque variante à un contrôle, avec une correction de Bonferroni pour tenir compte des comparaisons multiples.