...
Oui, les objets [!UICONTROL Recommendations] ([!UICONTROL Critères], [!UICONTROL Conceptions], [!UICONTROL Collections] et [!UICONTROL Exclusions]) créés via l’API sont disponibles dans l’interface utilisateur et peuvent être modifiés via l’API ou l’interface utilisateur [!DNL Target].
...
* Si un élément qui a été inclus précédemment doit maintenant être exclu, il l’est selon les « Attributs d’élément mis à jour... » Chronologie décrite ci-dessus en fonction de la source de flux (15 minutes via la mbox/l’API ou 12 à 24 heures via le flux).
...
## Combien de temps faut-il pour qu’une modification apportée à la configuration de mes paramètres [!UICONTROL Recommendations] d’activité, d’offre, de promotions ou de critères soit prise en compte sur mon site ?
...
## Pourquoi tous les critères, y compris les critères personnalisés, ne sont-ils pas disponibles pour sélection lors de la création d’une activité Recommandations ? {#section_B2265AC8B8A94E0298D495A05C5D817F}
...
Vous pourriez certainement essayer d’envoyer 50 000 produits en un seul appel. Si cela échoue, vous pouvez le diviser en lots. Adobe recommande aux clients de diviser leurs appels en lots de 5 000 ou 10 000 produits afin de réduire la probabilité d’une temporisation en raison de la charge du système.

## Dois-je spécifier le nom de la mbox lors de la création de règles de test de critère de Recommandations, de promotion ou de modèle ? {#section_FFA42ABCC5954B48A46526E32A3A88A2}

Lors de la création d’une règle de test de critère de Recommandations, de promotion ou de modèle basée sur un paramètre mbox, `mboxParameter` ne vous demande plus `mboxName`. Le nom de mbox est désormais optionnel. Cette modification vous permet d’utiliser les paramètres de plusieurs mbox ou de référencer un paramètre qui n’a pas encore été enregistré.
...
## Pourquoi ne puis-je pas enregistrer mon activité Recommandations héritée après avoir défini une nouvelle audience ? {#section_1E47C40B1FE7479BAC3EE0F50CE7C2C4}

Assurez-vous que le nom de l’audience est unique. Si le nom que vous lui avez donné est celui d’une audience déjà existante, vous ne pouvez pas enregistrer votre activité Recommandations héritée (activité Recommandations créée avant octobre 2016).
...
## Pourquoi mon action [!UICONTROL Télécharger des données] échoue-t-elle dans une activité [!DNL Recommendations] ? {#download-data-error}

Lorsque vous cliquez sur **[!UICONTROL Télécharger les données]** sur la page [!UICONTROL Aperçu de l’activité] d’une activité de [!DNL Recommendations], l’interface utilisateur de [!DNL Target] peut afficher le `Error while fetching recommendation data file.` d’erreur
...
## Que signifie la réponse NO_CONTENT parfois renvoyée dans la trace de contenu Recommandations ?
...
### Quels sont les problèmes connus lors de l’utilisation d’activités [!DNL Recommendations] ?
