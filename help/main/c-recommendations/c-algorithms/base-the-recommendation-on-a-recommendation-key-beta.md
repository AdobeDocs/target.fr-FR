...
description: Découvrez comment utiliser les recommandations basées sur des clés qui utilisent le contexte du comportement du visiteur pour afficher des résultats pertinents dans les activités [!UICONTROL Recommendations].
...
| Type d’algorithme | Quand utiliser les algorithmes disponibles |
| --- | --- |
| [!UICONTROL Cart-Based] | Effectuez des recommandations en fonction du contenu du panier de l’utilisateur.<ul><li>[!UICONTROL People Who Viewed These, Also Viewed]</li><li>[!UICONTROL People Who Viewed These, Also Bought]</li><li>[!UICONTROL People Who Bought These, Also Bought]</li></ul> |
...
Recommande les éléments achetés le plus souvent au cours de la même session que celle où l’élément spécifié est consulté.
...
Par exemple, l’affichage du surfboard A, puis du surfboard B dans une session donne A : 10, B : 5. Lorsque la session se termine, vous avez A : 5, B : 2,5. Si vous affichez les mêmes éléments lors de la session suivante, les valeurs deviennent A : 15 B : 7,5.
...
Par exemple, l’affichage de la catégorie A puis de la catégorie B dans une session donne A : 9, B : 10. Si vous affichez les mêmes éléments lors de la session suivante, les valeurs passent à A : 20, B : 9.
...
Si vous sélectionnez l’algorithme [!UICONTROL Most Viewed by Category], vous pouvez sélectionner les [!UICONTROL Recommendations Keys] suivants :
...
Si vous sélectionnez cet algorithme, vous pouvez sélectionner les [!UICONTROL Recommendations Keys] suivants :
...
Le type d’algorithme [!UICONTROL User-Based] vous permet de faire des recommandations en fonction du comportement de l’utilisateur.
...
Le type d’algorithme [!UICONTROL Custom Criteria] vous permet d’effectuer des recommandations sur la base d’un fichier personnalisé que vous chargez.
...
Avec l’ajout de règles d’inclusion sur [!UICONTROL Custom Criteria], les recommandations qui seraient statiques deviennent dynamiques et fondées sur les intérêts du visiteur.
...
Vous pouvez axer les recommandations sur la valeur d’un attribut de profil personnalisé. Supposons, par exemple, que vous souhaitiez afficher les films recommandés en fonction du film ajouté le plus récemment à la file d’attente par un visiteur.
...
1. Sélectionnez l’attribut d’entité correspondant à la clé choisie. Dans ce cas, &quot;[!UICONTROL Favorite Brand]&quot; correspond à `entity.brand`.

   [!DNL Recommendations] crée désormais une liste &quot;[!UICONTROL Top Sellers]&quot; pour chaque marque et affiche au visiteur la liste &quot;[!UICONTROL Top Sellers]&quot; appropriée en fonction de la valeur stockée dans l’attribut de profil [!UICONTROL Favorite Brand] du visiteur.
...
Recommande des éléments selon la certitude d’une relation entre ceux-ci. Vous pouvez configurer ce critère pour déterminer la quantité de données requises avant qu’une recommandation ne soit présentée à l’aide du curseur [!UICONTROL Inclusion Rules]. Par exemple, si vous sélectionnez très forte, les produits ayant la plus forte certitude de correspondance sont recommandés.
