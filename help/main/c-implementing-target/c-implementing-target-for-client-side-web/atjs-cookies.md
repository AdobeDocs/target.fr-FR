...
| Durée du cookie | Le cookie reste sur le navigateur du visiteur deux ans après la dernière connexion.<br>Le `deviceIdLifetime` peut être remplacé dans [at.js version 2.3.1 ou ultérieure](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank}. Pour plus d’informations, voir [targetGlobalSettings()](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}. |
